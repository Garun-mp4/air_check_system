...
- Если CO₂ и прогноз вернулись ниже комфортного порога и прошло минимальное время проветривания из настроек (по умолчанию 5 минут), автоматика создаёт batch-группу на закрытие окна и выключение обоих каналов.
- Ручная команда «Открыть» или «Закрыть» переводит окно в `manual` на выбранную в настройках длительность (15, 30 или 60 минут; по умолчанию 30). В этот период автоматика не перетирает решение пользователя.
...
Пороговые значения и длительности впервые записываются в настройки узла из `.env`, а затем редактируются оператором в панели «Настройки». Переменные `CO2_NORMAL_THRESHOLD`, `CO2_CRITICAL_THRESHOLD`, `PM25_GOOD_LIMIT`, `PM25_ELEVATED_LIMIT`, `WINDOW_MANUAL_OVERRIDE_MINUTES`, `AUTO_VENTILATION_MINIMUM_MINUTES`, `AUTOMATION_ENABLED`, `AUTO_WINDOW_ENABLED` и `ALERTS_ENABLED` используются как безопасные значения по умолчанию для нового узла.
...
3. **Сенсоры** — пространственно разделённые показатели внутри комнаты и снаружи, фото-контексты с плавным переходом в поверхность интерфейса, компактное сравнение разниц, PM2.5-шкалы и текущее окно.
...
Техническая информация не занимает место рядом с текущими показаниями: источник, PostgreSQL, модель прогноза и API-контракт открываются через кнопку **Настройки** в шапке. Внутри доступны вкладки «Автоматика», «Пороги и сигналы» и последняя вкладка «Технические сведения». Первые две действительно меняют постоянные правила узла, а техническая вкладка остаётся диагностическим разделом без управляющих действий; разовые команды остаются на вкладке «Управление».

В «Автоматике» можно включить или отключить автоматическое управление, разрешить автоматическое окно, выбрать длительность ручного режима и задать минимальное время проветривания. В «Порогах и сигналах» редактируются границы CO₂ и PM2.5; кнопка «Восстановить рекомендуемые значения» возвращает безопасный профиль интерфейса и автоматики, а «Отменить» отменяет только текущие несохранённые изменения.
...
| `PM25_GOOD_LIMIT` | верхняя граница нормальной зоны PM2.5 | `15` |
| `PM25_ELEVATED_LIMIT` | граница повышенной зоны PM2.5 | `35` |
...
| `AUTO_WINDOW_ENABLED` | разрешить автоматике открывать окно | `true` |
...
docker compose exec -T postgres psql -U air_quality -d air_quality -f /docker-entrypoint-initdb.d/004_node_settings.sql
```
